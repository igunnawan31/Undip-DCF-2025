    ...
    <footer className="relative bg-[#008080] text-white pt-16 pb-8 overflow-hidden">
      ...
      <div className="container mx-auto px-4 text-center relative z-10">
          ...
          <div className="flex space-x-6">
            {socialLinks.map((social) => (
              <Link 
                key={social.name} 
                href="#" 
                className="text-white hover:text-[#fdbe85] transition duration-300 flex items-center"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8 text-left">
          {[
            { title: 'Tentang', links: ['Sejarah', 'Visi Misi', 'Tim Kami', 'Testimoni'] },
            { title: 'Acara', links: ['Chemistry Competition', 'LKTI', 'Seminar Nasional', 'Workshop'] },
            { title: 'Dokumen', links: ['Guidebook', 'Poster', 'Template', 'Persyaratan'] },
            { title: 'Kontak', links: ['Email', 'WhatsApp', 'Alamat', 'Peta Lokasi'] }
          ].map((section) => (
            <div key={section.title}>
              ...
              <ul className="space-y-2">
                {section.links.map((link) => (
                  <li key={link}>
                    <Link href="#" className="text-white/80 hover:text-[#fdbe85] transition duration-300">
                      {link}
                    </Link>
                  </li>
                ))}
